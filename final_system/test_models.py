...
for vital, normal_val in normal_vitals.items():
    ...
    def run_and_collect(model, base_env, eval_env, max_steps=100, deterministic=True, seed=None, index=None):
        ...
        obs[idx] = normal_val # changed for feature ablation - set vital to normal value
        sev = reset_info['sev']
    ...
    for i in range(num_eps):
        seed = base_seed + i
        records, total_reward, map_violations, length, reset_info, final_info = run_and_collect(model, base_env, eval_env, max_steps=400, deterministic=True, seed=seed, index=i)
